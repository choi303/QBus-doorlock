			...
			if distance < maxDistance then
				...
				if isAuthorized then
					if doorID.locked then
						displayText = "~g~E~w~ - Kilitli"
					elseif not doorID.locked then
						displayText = "~g~E~w~ - Kilitli Degil"
					end
				elseif not isAuthorized then
					if doorID.locked then
						displayText = "Kilitli"
					elseif not doorID.locked then
						displayText = "Kilitli Degil"
					end
				end

				if doorID.locking then
					if doorID.locked then
						displayText = "Aciliyor.."
					else
						displayText = "Kilitleniyor.."
					end
				end
				...
				if QB.Doors[k].pickable then
					if QB.Doors[k].locked then
						if hasItem then
							...
							QBCore.Functions.Notify("toolkit yok moruk..", "error")
						end
					else
						QBCore.Functions.Notify('Kapi zaten kilitli degil??', 'error', 2500)
					end
				else
					QBCore.Functions.Notify('Bu kapıyı acmak cok zor', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function lockpickFinish(success)
    if success then
		QBCore.Functions.Notify('Basarılı!', 'success', 2500)
		setDoorLocking(closestDoorValue, closestDoorKey)
    else
        QBCore.Functions.Notify('Calismadi beya..', 'error', 2500)
    end
end
